...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Fetch profile first to get status
  ...
  const renderDashboardContent = () => {
    ...
    if (!profile) {
      return (
        <div className="text-center py-8">
          <p className="text-red-500">Profil konnte nicht geladen werden.</p>
        </div>
      );
    }
    ...
    switch (profile.status) {
      case 'bewerber':
        return <ApplicantDashboard application={application} loading={loading} />;
      case 'newcomer':
